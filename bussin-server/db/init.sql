...
CREATE UNIQUE INDEX IF NOT EXISTS trips_one_active_trip
...
CREATE INDEX IF NOT EXISTS trip_locations_latest_for_trip
...
CREATE TABLE IF NOT EXISTS parent_push_subscriptions (
  ...
  last_notified_trip_id uuid REFERENCES trips(id) ON DELETE SET NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS parent_push_subscriptions_enabled
